...
- Current state of browser verification: page load, both fighters initializing (x=288 / x=612), `mode:pve`, `live:false`, a 900×340 canvas, and no console errors are all confirmed; **a screenshot confirming the picture was not possible** — this environment's in-app browser has no visible viewport, rAF pauses while the tab is hidden, and the canvas has not produced a single drawn pixel. So "the stick figures are visible" was **not visually verified** this round and needs your confirmation in an open window.
  ...
  > **UPDATE — now visually verified, and it took a one-line fix.** Driving the page over CDP and pumping frames through the existing `globalThis.__game.pump()` hook produced the picture, under both the mock brain and the live API. It also exposed why the canvas had "not produced a single drawn pixel" here beyond rAF throttling: `View.stroke2` threw `TypeError: Cannot read properties of null (reading 'x')` on a frame where one pose point was missing, and because the throw happened inside `drawFighter`, everything after it was lost — the rest of that fighter's body *and every fighter drawn later*. Measured per half of the canvas, the right half went from **0 lit pixels to 1673** once the primitive was guarded. `stroke2` now returns early when either endpoint is not a finite point. Verified afterwards with a full live round: **zero exceptions**, 26-frame capture at `assets/images/fight_stance_triangle.gif`.
  >
  > Two notes for the record. First, the guard stops the throw but does not explain *which* pose point was missing on that frame; the underlying producer is still unidentified and should be found before this is called closed. Second, the fix had to be a finite-number test rather than a `null` test: `NaN` passes every `null` check and still poisons the canvas path, so `finitePoint()` covers both cases.
