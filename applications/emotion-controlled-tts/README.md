...
**Measured loudness, before any normalization.** Version B is **+1.7, +2.8 and +1.8 dB** louder than A across the three pairs (mean +2.1 dB). That is part of the effect rather than an artifact — the anger and authority axes raise energy — but it means a casual listen will partly hear *volume*. Normalize before judging timbre, which is what the figure below does.

![A/B spectrograms, normalized per pair](../../assets/images/ab_spectrograms.png)

Both files of each pair are rendered at the same loudness, so what is left on screen is timbre: in pair 2 the
Jev-controlled take carries visibly more energy in the 2–4 kHz band, which is the "vocal folds tighten, consonants
harden" claim above rendered as something you can check rather than take on trust. Confidence in the perceptual
description is the author's; the spectrograms are not. The figure is generated by
[`make_ab_visual.py`](make_ab_visual.py) from the six files in [`../../assets/audio`](../../assets/audio).
